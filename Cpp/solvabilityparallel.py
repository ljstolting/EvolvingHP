        ...
        if high_res == False: #then, conceivable to run through and check every point in the pyloric list
            ...
            while len(elimination_pyl_list>0):
                ...
                if np.min(dists_pyl)>tolerance:
                    ...
                    nonpyl_pt = nonpyloricavgs[np.where(dists_pyl==np.min(dists_pyl))[0][0]]
...
#ADD STABILITY CHECKS AND THEN SHARPEN PREDICTION TO WHETHER BEST PERFORMANCE SHOULD BE GOOD (SEPERABLE), MIXED (INSEPARABLE), OR BAD (UNSTABLE)
#FIRST ASSUME THE BOUNDARIES ARE AUTOMATICALLY CONSIDERED UNSTABLE. THE EXISTING "SOLVED IT BUT SHOULDN'T" ANOMALIES SHOULD SHOW UP. 

# print("recovery data gathered")

for i in range(15,16):
    solvability = uniquevals(i,tolerance = 0.005, evaluate=True,plot=False)
    with open('./solvability_output_test.dat', 'a') as f:
        print(i,file=f)
    ...
    with open('./solvability_readable_test.dat', 'a') as f:
